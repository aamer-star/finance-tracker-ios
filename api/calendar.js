module.exports = async (req, res) => {
  ...
  try {
    ...
    for (const line of lines.slice(1)) {
      ...
      const dateMs = new Date(reportDate + 'T20:00:00Z').getTime();
  ...
  if (apiKey) {
    ...
    for (const r of results) {
      ...
      for (const item of r.value.items) {
        ...
        if (dateMs >= cutoffMs) {
          events.push({ ticker: r.value.ticker, date: Math.floor(dateMs / 1000), type: 'earnings', epsEstimate: item.epsEstimate ?? null });
        }
      }
    }
    events.sort((a, b) => a.date - b.date);
